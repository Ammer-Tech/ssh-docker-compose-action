...
remote_command="set -e ; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/workspace\" ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/workspace\" ; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/workspace\" -xjv ; log 'Launching $DOCKER_COMPOSE...' ; cd \"\$HOME/workspace\" ; $DOCKER_COMPOSE -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" up -d --remove-orphans --build"
if $PULL; then
  remote_command="set -e; echo $PROJECT_VARS | xargs export; export AWS_ACCESS_KEY_ID=$REGISTRY_USER; export AWS_SECRET_ACCESS_KEY=$REGISTRY_PASSWORD; log() { echo '>> [remote]' \$@ ; } ; cleanup() { log 'Removing workspace...'; rm -rf \"\$HOME/workspace\" ; } ; log 'Creating workspace directory...' ; mkdir -p \"\$HOME/workspace\" ; trap cleanup EXIT ; log 'Unpacking workspace...' ; tar -C \"\$HOME/workspace\" -xjv ; log 'Launching $DOCKER_COMPOSE...' ; cd \"\$HOME/workspace\" ; log 'Docker login...'; /usr/local/bin/aws ecr get-login-password --region $AWS_DEFAULT_REGION | docker login -u AWS --password-stdin $DOCKER_REGISTRY; log 'Pull images...' ; $DOCKER_COMPOSE -f \"$DOCKER_COMPOSE_FILENAME\" pull ; $DOCKER_COMPOSE -f \"$DOCKER_COMPOSE_FILENAME\" -p \"$DOCKER_COMPOSE_PREFIX\" up -d --remove-orphans"
fi
